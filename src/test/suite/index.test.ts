...
suite("EditorConfig extension", function () {
	...
	test("indent_style = tab; tab_width = n", async () => {
		for (const n of [2, 3, 4]) {
			...
			assert.strictEqual(options.insertSpaces, false, `editor has insertSpaces: true`);
			assert.strictEqual(
				options.tabSize,
				n,
	...
	test("indent_style = space; indent_size = n", async () => {
		for (const n of [2, 3, 4]) {
			...
			assert.strictEqual(options.insertSpaces, true, `editor has insertSpaces: false`);
			assert.strictEqual(
				options.tabSize,
				n,
				`editor has a tabSize of ${options.tabSize} instead of ${n}`,
	...
	test("insert_final_newline = true", async () => {
		const savedText = await withSetting("insert_final_newline", "true").saveText("foo");
		assert.strictEqual(savedText, `foo${os.EOL}`, "editor fails to insert final newline on save");
	});

	test("insert_final_newline = false", async () => {
		const text = `foo${os.EOL}`;
		const savedText = await withSetting("insert_final_newline", "false").saveText(text);
		assert.strictEqual(savedText, text, "editor fails to preserve final newline on save");
	});

	test("insert_final_newline = unset", async () => {
		const text = `foo${os.EOL}`;
		const savedText1 = await withSetting("insert_final_newline", "unset").saveText(text);
		assert.strictEqual(savedText1, text, "editor fails to preserve final newline on save");

		const savedText2 = await withSetting("insert_final_newline", "unset-2").saveText("foo");
		assert.strictEqual(savedText2, "foo", "editor fails to preserve no final newline on save");
	});

	test("trim_trailing_whitespace = true", async () => {
		const savedText = await withSetting("trim_trailing_whitespace", "true").saveText("foo  ");
		assert.strictEqual(savedText, "foo", "editor fails to trim trailing whitespace on save");
	});

	test("trim_trailing_whitespace = false", async () => {
		const savedText = await withSetting("trim_trailing_whitespace", "false").saveText("foo  ");
		assert.strictEqual(savedText, "foo  ", "editor fails to preserve trailing whitespace on save");
	});

	test("trim_trailing_whitespace = unset", async () => {
		const savedText = await withSetting("trim_trailing_whitespace", "unset").saveText("foo  ");
		assert.strictEqual(savedText, "foo  ", "editor fails to preserve trailing whitespace on save");
	});
	...
	test("detect indentation (tab, empty root)", async () => {
		const options = await getOptionsForFixture(["detect-indentation", "root", "indent-style-tab"]);
	...
	test("detect indentation (tab, unset)", async () => {
		const options = await getOptionsForFixture(["detect-indentation", "unset", "indent-style-tab"]);
		const expectedTabSize = 4;
		assert.strictEqual(
			options.tabSize,
			expectedTabSize,
...
function withSetting(
	...
	return {
		async getText() {
			return (await this.createDoc(options.contents, options.fileName)).getText();
		},
		saveText(text: string) {
			return new Promise<string>(async (resolve) => {
				...
				assert.strictEqual(await workspace.applyEdit(edit), true, "editor fails to apply edit");
			});
		},
		async createDoc(contents = "", name = "test") {
			const uri = await utils.createFile(contents, getFixturePath([rule, value, name]));
